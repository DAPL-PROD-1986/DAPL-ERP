...
#         for row in self.plate_price_list:
...
#     return purchase_order[0].name if purchase_order else None
...
#         WHERE
#             po.docstatus < 2
#             AND poi.item_group IN %(categories)s
    ...
    def set_purchase_order_numbers(self):
        ...
        for table_field in child_tables:
            ...
            for row in rows:
                ...
                row.purchase_order_no = find_matching_purchase_order(
                    row.category,
                    row.supplier,
                    row.rate
                )
