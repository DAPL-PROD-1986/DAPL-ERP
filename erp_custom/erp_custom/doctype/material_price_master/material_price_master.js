...
function fetch_all_reference_purchase_orders(frm) {
    Object.keys(MATERIAL_PRICE_CONFIG).forEach(material => {
        fetch_reference_purchase_orders(frm, material);
    });
}
...
function fetch_reference_purchase_orders(frm, material) {
    ...
    if (!child_table.length) {
        set_reference_po_html(frm, config.html_field, "");
        return;
    }
    ...
    frappe.call({
        ...
        callback(r) {
            ...
            render_reference_purchase_orders(frm, material, purchase_orders);
        }
    });
}
...
function render_reference_purchase_orders(frm, material, purchase_orders) {
    const config = MATERIAL_PRICE_CONFIG[material];
    const groups = {};
    ...
    config.dimensions.forEach(dimension => {
        dimension_headers += `<th>${dimension[1]}</th>`;
    });
                ...
                <table class="rpo-table">
                    <thead>
                        <tr>
                            ...
                            <th colspan="${dimension_colspan}" style="text-align:center;"> Dimensions </th>
                            <th colspan="2" style="text-align:center;"> Weights </th>
                            <th colspan="2" style="text-align:center;"> Rate </th>
                        </tr>

                        <tr class="rpo-subhead"> ${dimension_headers}
                            <th> Kgs/Unit </th>
                            <th> Total Wt </th>
                            <th> Per Kg </th>
                            <th> Rate </th>
    ...
    if (!purchase_orders.length) {
        html += `
            <tr>
                <td colspan="${4 + dimension_colspan + 4}" class="rpo-empty"> No matching Purchase Orders found. </td>
            </tr>
        `;
    }
        ...
        category_names.forEach(category => {
            html += `
                <tr class="rpo-group-row">
                    <td colspan="${4 + dimension_colspan + 4}"> ${frappe.utils.escape_html(category)}
            ...
            groups[category].forEach(po => {
                    ...
                    <tr class="rpo-data">
                        <td>
                            <a class="rpo-po-link"
                                ...
                                target="_blank"> ${frappe.utils.escape_html(po.name || "")} </a>
                        </td>

                        <td> ${frappe.utils.escape_html(po.supplier || "")} </td>
                        <td class="rpo-num"> ${po.qty ?? ""} </td>
                        <td> ${frappe.utils.escape_html(po.custom_material_type || "")} </td>
                        <td> ${frappe.utils.escape_html(po.description || "")} </td>
                `;
                ...
                config.dimensions.forEach(dimension => {
                    ...
                    html += `<td class="rpo-num"> ${value} </td>`;
                });
    ...
    set_reference_po_html(frm, config.html_field, html);
}

// ===================== SET HTML FIELD ==================================
function set_reference_po_html(frm, fieldname, html) {
