...
#                 SUM(COALESCE(soi.amount, 0) + COALESCE(soi.igst_amount, 0) + COALESCE(soi.cgst_amount, 0) + COALESCE(soi.sgst_amount, 0)), 0) AS purchase_value,
#             COALESCE(
#                 GROUP_CONCAT(soi.item_code ORDER BY soi.idx SEPARATOR ', '), '') AS tag,

#             COALESCE(
#                 GROUP_CONCAT(soi.item_name ORDER BY soi.idx SEPARATOR ', '), '') AS item_name,

#             COALESCE(
#                 GROUP_CONCAT(soi.qty ORDER BY soi.idx SEPARATOR ', '), '') AS qty
...
# # ================================================================
# # PROJECT STATUS SPLIT DATA (Completed vs Open/Not Completed)
# # ================================================================
# # Used by the "Completed vs Open Projects" card below the table.
# # Respects the exact same filters (project, customer, status, type,
# # priority, portfolio, tag, fiscal year) as the rest of the dashboard,
# # then buckets the matching projects into two groups:
# #   - Completed        -> p.status == "Completed"
# #   - Not Completed     -> everything else (Open, On Hold, Cancelled, etc.)
# # ================================================================

# @frappe.whitelist()
# def get_project_status_split_data(filters=None):
...
#     # ============================================================
#     # PROJECT WISE VALUES (one row per project)
#     # ============================================================
...
#     # ============================================================
#     # SPLIT INTO COMPLETED / NOT COMPLETED
#     # ============================================================
...
#     completed_value = 0.0
#     not_completed_value = 0.0

#     for row in rows:

#         purchase_value = flt(row.get("purchase_value") or 0)
#         status = (row.get("status") or "").strip().lower()

#         if status == "completed":
#             completed_list.append(row)
#             completed_value += purchase_value

#         else:
#             not_completed_list.append(row)
#             not_completed_value += purchase_value
...
#         "not_completed_count": len(not_completed_list),

#         "completed_value": completed_value,

#         "not_completed_value": not_completed_value,
...
def get_project_dashboard_data(filters=None, limit=20, offset=0):
    ...
    totals = frappe.db.sql(f"""
        ...
        FROM (
            ...
            {where_clause}
            GROUP BY p.name) AS project_totals
    """, values, as_dict=True)[0]
...
# ================== DOWNLOAD PROJECT DASHBOARD EXCEL ==========================

@frappe.whitelist()
def download_project_excel(filters=None):
    ...
    conditions, values = build_project_conditions(filters, prefix="excel_")
    ...
    open_sheet = workbook.create_sheet("Open")
    ...
    light_blue_fill = PatternFill(fill_type="solid", fgColor="EFF6FF")
    ...
    def format_qty(value):
        ...
        for qty in quantities:
            ...
            try:
                ...
                formatted.append(qty)

        return ", ".join(formatted)

    def write_sheet(sheet, rows):
        sheet.freeze_panes = "A2"
        sheet.auto_filter.ref = f"A1:M{max(len(rows) + 1, 1)}"
        ...
        for row_index, project in enumerate(rows, start=2):

            values_list = [
                project.get("name") or "",
                project.get("customer") or "",
                project.get("item_code") or "",
                project.get("item_name") or "",
                # project.get("qty") or "",
                format_qty(project.get("qty")),
                project.get("portfolio") or "",
                project.get("status") or "",
                project.get("project_type") or "",
                project.get("priority") or "",
                project.get("fiscal_year") or "",
                flt(project.get("basic_value") or 0),
                flt(project.get("taxes") or 0),
                flt(project.get("purchase_value") or 0)
            ]

            for col_index, value in enumerate(values_list, start=1):
    ...
    # ======================== WRITE THREE TABS =============================
    write_sheet(all_sheet, projects)
    write_sheet(open_sheet, open_projects)
    write_sheet(completed_sheet, completed_projects)
