...
def get_project_dashboard_data(filters=None, limit=20, offset=0):
    ...
    totals = frappe.db.sql(f"""
        ...
        FROM (
            SELECT
                p.name,
                COALESCE(SUM(soi.amount), 0) AS basic_value,
                COALESCE(SUM(soi.igst_amount + soi.cgst_amount + soi.sgst_amount), 0) AS taxes,
                COALESCE(SUM(soi.amount + soi.igst_amount + soi.cgst_amount + soi.sgst_amount), 0) AS purchase_value
            FROM `tabProject` p
            LEFT JOIN `tabSales Order Item` soi
                ON soi.project = p.name
            {where_clause}
...
def get_project_chart_data(filters=None):
    ...
    portfolio_wise = frappe.db.sql(f"""
        SELECT
            ...
            COUNT(DISTINCT p.name) AS project_count,
            COALESCE(SUM(soi.amount + soi.igst_amount + soi.cgst_amount + soi.sgst_amount), 0) AS portfolio_value
        FROM `tabProject` p
        LEFT JOIN `tabSales Order Item` soi
            ON soi.project = p.name
        {where_clause}
