...
frappe.pages["project-dashboard"].on_page_load = function (wrapper) {
    ...
    $(`
        <style>
            ...
            .project-status-column-title {
                font-size: 14px;
                font-weight: 700;
                margin-bottom: 10px;
            }

            .project-status-column-title.open-title {
                color: #a16207;
            }

            .project-status-column-title.completed-title {
                color: #15803d;
            }

            .project-status-ratio-card {
                display: flex;
                align-items: center;
                justify-content: space-between;
                gap: 12px;
            ...
            .project-status-ratio-info {
                display: flex;
                flex-direction: column;
                gap: 2px;
            }

            .project-status-ratio-label {
                font-size: 11px;
                color: #64748b;
            }

            .project-status-ratio-count {
                font-size: 22px;
                font-weight: 700;
                color: #1e293b;
            }
            ...
            .project-status-ratio.open-ratio {
                color: #d97706;
            }

            .project-status-ratio.completed-ratio {
                color: #16a34a;
            }
            ...
            .project-download-excel:hover {
                background: #f1f5f9;
                color: #1e293b;
            }

            .project-download-excel svg {
                width: 16px;
                height: 16px;
            }
    ...
    $(`
        <div class="project-dashboard">
            ...
            <div class="project-card-row">
                <div class="project-dashboard-card project-card-blue">
                    <div class="project-card-title"> ${__("Total Projects (Items)")} </div>
                    <div class="project-card-values">
                        <div class="project-card-count" id="project-total-count"> 0 </div>
                        <div class="project-card-overall-value" id="project-total-overall-value"> ₹0.00 </div>
                    </div>
        ...
        <div class="project-dashboard-section project-status-split-section">
            <div class="project-section-header">
                <div>
                    <div class="project-section-title"> <i class="fa fa-pie-chart"></i>
                        <span>${__("Completed vs Open Projects")}</span> </div>
                    <div class="project-chart-subtitle"> ${__("Values include GST")} </div>
                </div> 
            </div>
                ...
                <div class="project-status-split-body" id="project-status-split-body">
                    ...
                    <div class="project-status-column">
                        <div class="project-status-column-header">
                            <div class="project-status-column-title open-title">
                                ${__("Open / Not Completed")}
                            </div>

                            <div class="project-status-ratio-card">
                                <div class="project-status-ratio-info">
                                    <span class="project-status-ratio-label">
                                        ${__("Projects")}
                                    </span>
                                    <span class="project-status-ratio-count" id="project-open-count">
                                        0
                                    </span>
                                </div>

                                <div class="project-status-ratio open-ratio" id="project-open-ratio">
                                    0%
                                </div>
                            </div>
                        </div>

                        <div class="project-status-list" id="project-open-list"></div>
                    ...
                    <div class="project-status-column">
                        ...
                        <div class="project-status-column-header">
                            <div class="project-status-column-title completed-title">
                                ${__("Completed")}
                            </div>
                            ...
                            <div class="project-status-ratio-card">
                                <div class="project-status-ratio-info">
                                    ...
                                    <span class="project-status-ratio-count" id="project-completed-count">
                                        0
                                    </span>
                                </div>

                                <div class="project-status-ratio completed-ratio" id="project-completed-ratio">
                                    0%
                                </div>
                            </div>
                        </div>

                        <div class="project-status-list" id="project-completed-list"></div>
            ...
            <div class="project-dashboard-section">
                <div class="project-section-header">
                    <div class="project-section-title">
                        <i class="fa fa-bar-chart"></i> <span>${__("Projects")}</span>
                    </div>

                    <button class="project-download-excel" id="project-download-excel" title="${__("Download Excel")}">
    ...
    function render_project_status_split(data) {
        ...
        const completed_pct = total_count ? ((completed_count / total_count) * 100).toFixed(1) : "0.0";
        const not_completed_pct = total_count ? ((not_completed_count / total_count) * 100).toFixed(1) : "0.0";

        $("#project-open-count").text(not_completed_count);
        $("#project-open-ratio").text(`${not_completed_pct}%`);

        $("#project-completed-count").text(completed_count);
        $("#project-completed-ratio").text(`${completed_pct}%`);

        render_status_list("#project-open-list", data.not_completed_list || []);
        render_status_list("#project-completed-list", data.completed_list || []);
    }

    function render_status_list(selector, rows) {
        const container = $(selector);
        container.empty();

        if (!rows.length) {
            container.html(`<div class="project-empty">${__("No data found.")}</div>`);
            return;
        }

        rows.forEach((row, index) => {
            const item_codes = format_text_list(row.item_code);
            const item_names = format_text_list(row.item_name);

            container.append(`
                <div class="project-status-item">
                    <div class="project-status-number">${index + 1}</div>

                    <div class="project-status-item-info">
                        <div class="project-status-item-main">
                            <span class="project-status-item-project" title="${frappe.utils.escape_html(row.name || "")}">
                                ${frappe.utils.escape_html(row.name || "-")}
                            </span>

                            <span class="project-status-item-name" title="${frappe.utils.escape_html(item_names)}">
                                ${frappe.utils.escape_html(item_names)}
                            </span>
                        </div>

                        <span class="project-status-item-code" title="${frappe.utils.escape_html(item_codes)}">
                            ${frappe.utils.escape_html(item_codes)}
                        </span>
                    ...
                    <div class="project-status-item-value">
                        ${format_currency(chart_flt(row.purchase_value))}
                    </div>
                </div>
            `);
        });
    ...
    function format_text_list(raw_value) {
        if (!raw_value || raw_value === "-") {
            return "-";
        }
    ...
    function render_project_table(
        ...
        if (reset) {
            ...
            container.html(`
                ...
                <!-- FOOTER -->
                <div class="project-table-footer">
                    <div class="project-footer-left"> <span> ${__("Show")} </span>
                        <select id="project-page-size" class="project-page-size">
                            <option value="20"> 20 </option>
                            <option value="100"> 100 </option>
                            <option value="500"> 500 </option>
    ...
    // ====================== DOWNLOAD EXCEL =========================
    $(document).on("click", "#project-download-excel", function () {
        const button = $(this);

        button.prop("disabled", true);

        frappe.call({
            method: "erp_custom.erp_custom.page.project_dashboard.project_dashboard.download_project_excel",
            args: {
                filters: JSON.stringify(current_filters)
            },
            callback: function (r) {
                ...
                if (!r.message) {
                    frappe.msgprint(__("Unable to generate Excel file."));
                    return;
                }
                ...
                link.href = file_url;
                link.download = "Project Dashboard.xlsx";
